...
from models.DeepTransformer import TransformerModel1d
...
wandb.init(
    ...
    notes="Deep Transformer",
)

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# Create the model
# model = LSTMModel1d(num_classes=1, input_channels=8, nh=128, nlayers=2, depth=6).to(device)
model = TransformerModel1d(num_classes=1, input_channels=8, nh=128, nheads=8, nlayers=2, depth=6).to(device)
