...
class PTB_XL_PLUS_ECGDataset(Dataset):
    def __init__(self, parameter=None):
        ...
        self.X = torch.tensor(self.X, dtype=torch.float32) # there is a waring saying this is slow. but we need them as separate tensors so...
        ...
        if parameter == HR_PARAMETER: 
            ...
            hr_torch = torch.tensor(self.features_df['RR_Mean_Global'].values, dtype=torch.float32) 
            # calculate HR
            self.y = 60 * 1000/hr_torch
            
        elif parameter == QRS_PARAMETER:
            self.features_df = self.features_df.dropna(subset=['QRS_Dur_Global']) 
            self.y = torch.tensor(self.features_df['QRS_Dur_Global'].values, dtype=torch.float32)

        elif parameter == PR_PARAMETER:
            self.features_df = self.features_df.dropna(subset=['QT_Int_Global']) 
            self.y = torch.tensor(self.features_df['QT_Int_Global'].values, dtype=torch.float32)
        
        elif parameter == QT_PARAMETER: 
            self.features_df = self.features_df.dropna(subset=['PR_Int_Global'])
            self.y = torch.tensor(self.features_df['PR_Int_Global'].values, dtype=torch.float32)
