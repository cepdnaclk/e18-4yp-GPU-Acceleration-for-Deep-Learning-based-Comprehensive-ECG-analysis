...
from models.SimpleTransformerIshan import TransformerModel
...
wandb.init(
    ...
    notes="test 2 with lr 0.01",
)

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# Create the model
input_dim = 5000  # Number of channels in the input
output_dim = 1  # Output dimension (for HR_PARAMETER)
num_heads = 8
num_layers = 6
model = TransformerModel(input_size=5000, hidden_size=8, num_layers=2, num_heads=8, output_size=1).to(device)

# Create the dataset class
dataset = Deepfake_ECG_Dataset(parameter=parameter, output_type=CH_8_2D_MATRIX_OUTPUT_TYPE)

# Split the dataset into training and validation sets
train_indices, val_indices = train_test_split(range(len(dataset)), test_size=1 - train_fraction, random_state=SEED)
...
for epoch in range(num_epochs):
    ...
    try:
        ...
        # Print the error message here divide by zero error occurred
