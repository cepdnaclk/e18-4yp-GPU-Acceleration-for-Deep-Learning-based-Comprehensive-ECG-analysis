...
logging.basicConfig(
    filename='log_classification.log',  # Log file name
...
subsets = [SUB_DATASET_A, SUB_DATASET_B]
NUMBER_OF_TOTAL_RUNS = 3

for current_run_number in range(NUMBER_OF_TOTAL_RUNS):
    for current_subset in subsets:
        ...
        select_sub_dataset = current_subset
        ...
        wandb.init(
            ...
            notes=f"#{current_run_number+1} run in subset {current_subset}.",
        )
        ...
        logging.info(f"start training of #{current_run_number+1} run in subset {current_subset}")
        try:
            ...
            wandb.finish()
            logging.info(f"end training of #{current_run_number+1} run in subset {current_subset}")
        except Exception as e:
            logging.error(f"Error in #{current_run_number+1} and {current_subset} and the error is :{e}")
            try:
                wandb.finish(-1)
            except:
